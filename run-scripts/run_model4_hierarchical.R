...
pprev_base <- function(gamma, sigma, age) (gamma/(gamma+sigma))*(1 - exp(-(gamma + sigma)*age))
...
for(s in 1:n_datasets){
  ...
  for(a in 1:n_ages){
    pred_prev_base[s,a] <- pprev_base(gamma = gamma[s], age = ages_data[s,a], sigma = sigma)
...
pprev <- function(gamma, age, sigma, omega, M_initial){
  (gamma/(gamma+sigma))*(1 - exp(-(gamma + sigma)*age)) - 
    M_initial*((gamma/(gamma + sigma - omega))*(exp(-omega*age) - exp( - (gamma + sigma)*age)))
  }
...
for(s in 1:n_datasets){
  ...
  for(a in 1:n_ages){
    pred_prev[s,a] <- pprev(gamma = gamma[s], age = ages_data[s,a], sigma = sigma, omega = omega, M_initial = M_initial[s])
...
fit_model4 <- stan(
  file = here::here("stan-models/model4_hierarchical.stan"),
  data = list(
    S = nrow(ages_data),
    A =  ncol(ages_data),
    N = N_camels,
